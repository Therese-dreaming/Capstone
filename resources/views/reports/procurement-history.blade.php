    ...
    <div class="p-6">
        ...
        <div class="bg-white rounded-lg shadow-lg p-6">
            ...
            <div class="flex justify-between items-center mb-6">
                <div>
                    <h1 class="text-2xl font-bold">Procurement History</h1>
                    <p class="text-gray-600">Total Assets Procured: {{ $assets->count() }}</p>
                </div>
                <div class="flex space-x-4">
                    <!-- Date Filter Form -->
                    <form action="{{ route('reports.procurement-history') }}" method="GET" class="flex space-x-4" id="dateFilterForm">
                        <div class="flex items-center space-x-2">
                            <label for="start_date" class="text-sm font-medium text-gray-600">From:</label>
                            <input type="date" id="start_date" name="start_date" value="{{ request('start_date') }}" onchange="this.form.submit()" class="border-gray-300 rounded-md shadow-sm focus:border-red-500 focus:ring-red-500">
                        </div>
                        <div class="flex items-center space-x-2">
                            <label for="end_date" class="text-sm font-medium text-gray-600">To:</label>
                            <input type="date" id="end_date" name="end_date" value="{{ request('end_date') }}" onchange="this.form.submit()" class="border-gray-300 rounded-md shadow-sm focus:border-red-500 focus:ring-red-500">
                        </div>
                        @if(request('start_date') || request('end_date'))
                        <a href="{{ route('reports.procurement-history') }}" class="bg-gray-500 text-white px-4 py-2 rounded-md hover:bg-gray-600">
                            Reset
                        </a>
                        @endif
                    </form>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $asset->purchase_date }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">₱{{ number_format($asset->purchase_price, 2) }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $asset->vendor }}</td>
                    ...
                    <tfoot class="bg-gray-50">
                        <tr>
                            <td colspan="4" class="px-6 py-4 text-sm font-medium text-gray-900">Total Procurement Value</td>
