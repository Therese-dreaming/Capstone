    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        <div class="p-6 space-y-4">
            ...
            @php
                ...
                // Explicitly set the timezone to Asia/Manila when parsing the date
                $formattedDate = \Carbon\Carbon::parse($actionDate)->setTimezone('Asia/Manila')->format('Y-m-d');
                
                // Check if this is a new date group
                if (!isset($currentDate) || $currentDate !== $formattedDate) {
            ...
            @if($showDateHeader)
                <div class="border-b border-gray-200 pb-1 mb-3">
                    <h3 class="text-md font-medium text-gray-700">{{ \Carbon\Carbon::parse($actionDate)->setTimezone('Asia/Manila')->format('F j, Y') }}</h3>
                </div>
            @endif
            ...
            <div class="p-4 border-l-4 {{ $action->action_source === 'asset_history' ? 'border-blue-500 bg-blue-50' : ($action->action_source === 'repair' ? 'border-green-500 bg-green-50' : 'border-purple-500 bg-purple-50') }} rounded-lg shadow-sm hover:shadow transition duration-150 ease-in-out">
                <div class="flex justify-between items-center">
                    ...
                    <span class="text-sm {{ $action->action_source === 'asset_history' ? 'text-blue-600' : ($action->action_source === 'repair' ? 'text-green-600' : 'text-purple-600') }}">
                        <span class="text-sm {{ $action->action_source === 'asset_history' ? 'text-blue-600' : ($action->action_source === 'repair' ? 'text-green-600' : 'text-purple-600') }}">
                            {{ \Carbon\Carbon::parse($action->action_source === 'asset_history' ? $action->created_at : $action->completed_at)->setTimezone('Asia/Manila')->format('g:i A') }}
                        </span>
                    </span>
                </div>
