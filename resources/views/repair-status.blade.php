        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6" id="requestsCards">
            ...
            <div class="bg-white rounded-xl shadow-lg p-4 sm:p-6 flex flex-col gap-3 sm:gap-4 border border-gray-200 hover:shadow-xl transition-all duration-200">
                ...
                <div class="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-3 sm:gap-2">
                    ...
                    <div class="flex flex-wrap sm:flex-col items-start sm:items-end gap-1 sm:gap-2">
                        <!-- Urgency Level Badge -->
                        ...
                        @if(($request->verification_status ?? null) === 'disputed')
                        <!-- Rework Requested Badge -->
                        <span class="inline-flex items-center px-2 sm:px-3 py-1 sm:py-1.5 rounded-full text-xs font-semibold whitespace-nowrap bg-orange-100 text-orange-800 border border-orange-200" title="Caller requested rework">
                            <svg class="w-3 h-3 mr-1 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                            </svg>
                            <span class="hidden sm:inline">Rework Requested</span>
                            <span class="sm:hidden">Rework</span>
                        </span>
                        @endif

                        @if(($request->rework_count ?? 0) > 0)
                        <!-- Rework Count Badge -->
                        <span class="inline-flex items-center px-2 sm:px-3 py-1 sm:py-1.5 rounded-full text-xs font-semibold whitespace-nowrap bg-purple-100 text-purple-800 border border-purple-200" title="Times reworked">
                            <svg class="w-3 h-3 mr-1 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20"><path d="M10 2a8 8 0 100 16 8 8 0 000-16zm1 11H9v-4h2v4zm0-6H9V5h2v2z"/></svg>
                            <span class="hidden sm:inline">Reworks: </span>{{ $request->rework_count }}
                        </span>
                        @endif
                    </div>
                </div>
                ...
                <div class="flex flex-wrap gap-1 sm:gap-2 mt-auto pt-3 sm:pt-4 border-t border-gray-200">
                    ...
                    @elseif(!$request->time_started)
                        @if($request->technician_id == auth()->id())
                        ...
                        @else
                        <button disabled class="bg-gray-400 text-white p-2 rounded-lg cursor-not-allowed" title="Complete (Assigned to {{ $request->technician->name }})">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 13l-3-3m0 0l-3 3m3-3v12M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9" />
                            </svg>
                        </button>
                        <button disabled class="bg-gray-400 text-white p-2 rounded-lg cursor-not-allowed" title="Start Repair (Assigned to {{ $request->technician->name }})">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z" />
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                            </svg>
