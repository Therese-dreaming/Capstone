    ...
    <div class="max-w-7xl mx-auto px-6 py-6">
        ...
        <div class="mb-6">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900">Available Assets</h1>
                    <p class="text-sm text-gray-600 mt-1">Browse and manage borrowable equipment</p>
                </div>
                <div class="flex flex-wrap items-center gap-3">
                    <span class="px-3 py-1.5 bg-gradient-to-r from-green-500 to-green-600 text-white rounded-lg text-sm font-semibold shadow-sm">
        ...
        <div class="bg-white rounded-xl shadow-md p-5 mb-6 border border-gray-100">
            <form method="GET" action="{{ route('borrowing.assets.available') }}" class="space-y-4 md:space-y-0 md:flex md:items-end md:gap-3">
                <!-- Search -->
                <div class="flex-1">
                ...
                <div class="flex gap-2">
                    ...
                    <a href="{{ route('borrowing.assets.available') }}" class="px-5 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 text-sm font-semibold rounded-lg transition-all">
                        Reset
                    </a>
                </div>
            </form>
                        ...
                        <div class="p-4">
                            ...
                            <div class="space-y-2 mb-4">
                                ...
                                <div class="flex items-start gap-2">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <p class="text-xs text-gray-500 font-medium">Model</p>
                                        <p class="text-sm text-gray-900 truncate" title="{{ $asset->model }}">
                                            {{ $asset->model }}
                                        </p>
                                    </div>
                                </div>
                            ...
                            <!-- Action Button -->
                            <button class="w-full py-2.5 bg-gradient-to-r from-red-600 to-red-700 hover:from-red-700 hover:to-red-800 text-white text-sm font-semibold rounded-lg transition-all shadow-sm hover:shadow-md group-hover:scale-105">
                                Borrow Now
                            </button>
                        </div>
                    </div>
        ...
        @else
            <div class="bg-white rounded-xl shadow-md p-12 text-center border border-gray-100">
                ...
                <h3 class="text-lg font-bold text-gray-700 mb-2">No Available Assets Found</h3>
                <p class="text-sm text-gray-600 mb-6">
                    @if(request()->has('search') || request()->has('category') || request()->has('location'))
                        Try adjusting your filters to see more results.
                    @else
                        There are currently no assets available for borrowing.
                    @endif
                </p>
                @if(request()->has('search') || request()->has('category') || request()->has('location'))
                    <a href="{{ route('borrowing.assets.available') }}" class="inline-flex items-center px-5 py-2.5 bg-gradient-to-r from-red-600 to-red-700 hover:from-red-700 hover:to-red-800 text-white text-sm font-semibold rounded-lg transition-all shadow-sm hover:shadow-md">
                        Clear Filters
                    </a>
                @endif
            </div>
