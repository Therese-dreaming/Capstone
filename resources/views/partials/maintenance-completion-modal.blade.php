...
<div id="completionModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden overflow-y-auto h-full w-full z-50">
    <div class="relative top-20 mx-auto p-5 border w-[600px] shadow-lg rounded-lg bg-white">
        <!-- Close button -->
        <button onclick="hideModal('completionModal')" class="absolute top-4 right-4 text-gray-400 hover:text-gray-600 focus:outline-none">
        ...
        <div class="mt-3">
            ...
            <div class="flex items-center justify-center mb-6">
                <svg class="w-8 h-8 text-red-800 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
                <h3 class="text-xl font-bold text-gray-900">Complete Maintenance</h3>
            </div>
...
async function searchAsset() {
    ...
    function addAnotherIssue() {
        ...
        issueDiv.className = 'space-y-3 border-t pt-4';
        issueDiv.innerHTML = `
        <div class="space-y-2">
            <div class="flex space-x-2">
                <div class="flex-1">
                    <label class="block text-sm font-medium text-gray-700">Search Asset by Serial Number</label>
                    <input type="text" class="serial_number mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500" name="additional_serial_number[]">
                </div>
                <div class="flex items-end">
                    <button type="button" onclick="searchAdditionalAsset(this)" class="px-4 py-2 bg-red-800 text-white rounded-md hover:bg-red-700">Search</button>
                </div>
            </div>
            <div class="assetMessage hidden">
                <p class="message-title font-medium"></p>
                <p class="message-content"></p>
            </div>
        </div>
        <div>
            <label class="block text-sm font-medium text-gray-700">Asset Name</label>
            <input type="text" class="asset_name mt-1 block w-full rounded-md border-gray-300 bg-gray-50 shadow-sm" readonly name="additional_asset_name[]">
        </div>
        <div>
            <label class="block text-sm font-medium text-gray-700">Category</label>
            <input type="text" class="asset_category mt-1 block w-full rounded-md border-gray-300 bg-gray-50 shadow-sm" readonly name="additional_asset_category[]">
        </div>
        <div>
            <label class="block text-sm font-medium text-gray-700">Location</label>
            <input type="text" class="asset_location mt-1 block w-full rounded-md border-gray-300 bg-gray-50 shadow-sm" readonly name="additional_asset_location[]">
        </div>
        <div>
            <label class="block text-sm font-medium text-gray-700">End of Life Date</label>
            <input type="text" class="asset_eol mt-1 block w-full rounded-md border-gray-300 bg-gray-50 shadow-sm" readonly name="additional_asset_eol[]">
        </div>
        <div>
            <label class="block text-sm font-medium text-gray-700">Issue Description</label>
            <textarea class="issue_description mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500" rows="3" name="additional_issue_description[]"></textarea>
        </div>
    `;
    ...
    async function searchAdditionalAsset(button) {
        const issueDiv = button.closest('.space-y-3');
        ...
        try {
            ...
            issueDiv.querySelector('.asset_name').value = data.name || '';
            issueDiv.querySelector('.asset_category').value = data.category?.name || '';
            issueDiv.querySelector('.asset_location').value = data.location || '';
            issueDiv.querySelector('.asset_eol').value = data.end_of_life_date || 'Not specified';
            ...
            issueDiv.querySelector('.asset_name').value = '';
            issueDiv.querySelector('.asset_category').value = '';
            issueDiv.querySelector('.asset_location').value = '';
            issueDiv.querySelector('.asset_eol').value = '';
        }
    }
    ...
    document.getElementById('completeForm').addEventListener('submit', function(e) {
        if (document.querySelector('input[name="has_issues"]:checked').value === '1') {
            e.preventDefault();
            ...
            // Get main issue
            const mainIssueDescription = document.querySelector('textarea[name="issue_description"]').value;
            const mainSerialNumber = document.getElementById('serial_number').value;

            if (!mainSerialNumber || !mainIssueDescription) {
                alert('Please fill in both serial number and issue description for the main issue');
                return;
            }
            ...
            Array.from(additionalIssues).forEach(issueDiv => {
                const serialNumber = issueDiv.querySelector('.serial_number').value;
                const issueDescription = issueDiv.querySelector('.issue_description').value;
            ...
            // Add serial number and issues to form data
            const serialNumberInput = document.createElement('input');
            serialNumberInput.type = 'hidden';
            serialNumberInput.name = 'serial_number';
            serialNumberInput.value = mainSerialNumber;
            this.appendChild(serialNumberInput);
