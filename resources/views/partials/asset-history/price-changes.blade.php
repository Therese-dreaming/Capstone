<div class="mb-8">
    @php
        ...
        $currentPageRecords = $priceChanges->slice($offset, $perPage);
    @endphp
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50 transition-colors duration-200">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                        <span class="px-3 py-1.5 text-xs font-medium rounded-full inline-flex items-center justify-center min-w-[90px] bg-orange-100 text-orange-800">
                            ₱{{ number_format($record->old_value, 2) }}
                        </span>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                        <span class="px-3 py-1.5 text-xs font-medium rounded-full inline-flex items-center justify-center min-w-[90px] bg-orange-100 text-orange-800">
                            ₱{{ number_format($record->new_value, 2) }}
                        </span>
                    </td>
                ...
                <tr>
                    <td colspan="5" class="px-6 py-8 text-center text-gray-500 text-sm bg-gray-50">
                        <div class="flex flex-col items-center justify-center">
                            ...
                            <span>No price changes recorded</span>
                        </div>
                    </td>
                </tr>
                @endforelse
    ...
    <!-- Price Change Cards (Mobile View) -->
    <div class="md:hidden">
        @php
            // Sort price changes by date (newest first) and apply pagination
            $sortedPriceChanges = $currentPageRecords->sortByDesc('created_at');

            // Group price changes by day
            $priceChangesByDay = $sortedPriceChanges->groupBy(function($record) {
                return $record->created_at->format('Y-m-d');
            });
        @endphp

        @forelse($priceChangesByDay as $day => $records)
            @php
                    ...
                    @foreach($records as $record)
                        <div class="bg-white rounded-lg shadow p-4">
                            ...
                            <div class="mb-2">
                                ...
                                <span class="px-2 py-1 text-xs font-medium rounded-full inline-flex items-center justify-center bg-orange-100 text-orange-800">
                                    ₱{{ number_format($record->old_value, 2) }}
                                </span>
                            </div>
                            <div class="mb-3">
    ...
    @if($totalPages > 1)
        <div class="mt-8 flex items-center justify-between">
            <div class="text-sm text-gray-700">
                Showing {{ ($currentPage - 1) * $perPage + 1 }} to {{ min($currentPage * $perPage, $totalRecords) }} of {{ $totalRecords }} price changes
            </div>
            <div class="flex items-center space-x-2">
                @if($currentPage > 1)
                    <a href="?price_page={{ $currentPage - 1 }}&page={{ request()->get('page') }}&start_date={{ request()->get('start_date') }}&end_date={{ request()->get('end_date') }}&active_tab={{ request()->get('active_tab', 'timeline') }}" class="px-3 py-2 text-sm font-medium text-gray-500 bg-white border border-gray-300 rounded-md hover:bg-gray-50 hover:text-gray-700 transition-colors">
                        Previous
                    </a>
                @endif
                ...
                @for($i = max(1, $currentPage - 2); $i <= min($totalPages, $currentPage + 2); $i++)
                    ...
                    @else
                        <a href="?price_page={{ $i }}&page={{ request()->get('page') }}&start_date={{ request()->get('start_date') }}&end_date={{ request()->get('end_date') }}&active_tab={{ request()->get('active_tab', 'timeline') }}" class="px-3 py-2 text-sm font-medium text-gray-500 bg-white border border-gray-300 rounded-md hover:bg-gray-50 hover:text-gray-700 transition-colors">
                            {{ $i }}
                        </a>
                    @endif
                @endfor
