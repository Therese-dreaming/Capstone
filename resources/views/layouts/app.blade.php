        ...
        <aside class="w-72 bg-red-800 text-white min-h-screen fixed left-0 top-16 pt-10">
            <div class="p-4 pl-8">
                ...
                <nav class="space-y-3">
                    ...
                    @if(auth()->user()->group_id <= 2) <a href="{{ route('my.tasks') }}" class="font-bold flex items-center px-6 py-2.5 text-white-500 hover:bg-red-50 hover:text-red-700 rounded-lg transition-colors duration-200">
                    ...
                    .then(data => {
                        notificationList.innerHTML = data.length ?
                            data.map(notification => `
                                <div class="p-4 border-b border-gray-100 hover:bg-gray-50 ${notification.is_read ? '' : 'bg-red-50'}" 
