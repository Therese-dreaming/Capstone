...
<div class="flex-1 p-4 md:p-8">
    ...
    <div class="mb-4 md:mb-6">
        <a href="{{ route('maintenance.history') }}" class="inline-flex items-center text-gray-600 hover:text-red-800 transition-colors duration-200 text-sm md:text-base">
            <svg class="w-4 h-4 md:w-5 md:h-5 mr-1 md:mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 17l-5-5m0 0l5-5m-5 5h12" />
            </svg>
            <span>Back to Maintenance History</span>
        </a>
    </div>
        ...
        <div class="bg-red-800 rounded-xl shadow-lg p-4 md:p-6 text-white">
            <div class="flex items-center justify-between">
                <div class="flex items-center">
                    ...
                    <div>
                        <h1 class="text-2xl md:text-3xl font-bold text-white mb-1 md:mb-2">Maintenance Details</h1>
                        <p class="text-red-100 text-sm md:text-lg">Complete information for this maintenance record</p>
                    </div>
                </div>
                <div class="flex space-x-2">
                    <button onclick="confirmDelete({{ $maintenance->id }})" class="inline-flex items-center px-4 py-2 bg-red-600 text-white font-medium rounded-lg hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 focus:ring-offset-red-800 transition-colors duration-200">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="flex items-center space-x-3">
                        ...
                        <div>
                            ...
                            <p class="text-sm font-semibold text-gray-900">{{ \Carbon\Carbon::parse($maintenance->scheduled_date)->format('M d, Y') }}</p>
                        </div>
                    </div>
                    <div class="flex items-center space-x-3">
                        ...
                        <div>
                            ...
                            <p class="text-sm font-semibold text-gray-900">{{ optional($maintenance->target_date)->format('M d, Y') ?? 'Not specified' }}</p>
                        </div>
                    </div>
                    <div class="flex items-center space-x-3">
                    ...
                    <div class="flex items-center space-x-3">
                        ...
                        <div>
                            ...
                            <p class="text-sm font-semibold text-gray-900">{{ $maintenance->technician->name }}</p>
                        </div>
                    </div>
                    <div class="flex items-center space-x-3">
                        ...
                        <div>
                            ...
                            <p class="text-sm font-semibold text-gray-900">{{ $maintenance->actionBy ? $maintenance->actionBy->name : 'System' }}</p>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center space-x-3">
                        ...
                        <div>
                            ...
                            <p class="text-sm font-semibold text-gray-900">{{ \Carbon\Carbon::parse($maintenance->completed_at)->format('M d, Y g:i A') }}</p>
                        </div>
                    </div>
                    @endif
                    ...
                    <div class="flex items-center space-x-3">
                        ...
                        <div>
                            ...
                            <p class="text-sm font-semibold text-gray-900">{{ $maintenance->lab_number }}</p>
                        </div>
                    </div>
                    @endif
                </div>
                ...
                @if($maintenance->notes)
                    <div class="bg-gradient-to-r from-purple-50 to-purple-100 rounded-lg p-4 border border-purple-200">
                        <div class="flex items-start space-x-3">
                            <div class="bg-purple-200 p-2 rounded-full mt-1">
                                <svg class="w-4 h-4 text-purple-700" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                </svg>
                            </div>
                            <p class="text-sm text-gray-700 whitespace-pre-wrap flex-1">{{ $maintenance->notes }}</p>
                        </div>
                    </div>
                @else
                    <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                        <div class="flex items-center space-x-3">
                            ...
                            <p class="text-sm text-gray-500 italic">No notes were added for this maintenance record</p>
                        </div>
                    </div>
                @endif
            </div>
                    ...
                    <div class="space-y-3">
                        @foreach($tasks as $index => $task)
                            <div class="flex items-center space-x-3 p-3 bg-green-50 rounded-lg border border-green-200">
                                ...
                                <p class="text-sm text-gray-700 flex-1">{{ $task }}</p>
                            </div>
                        @endforeach
                    </div>
                @else
                    <div class="bg-green-50 rounded-lg p-4 border border-green-200">
                        <p class="text-sm text-gray-700">{{ $maintenance->maintenance_task }}</p>
                    </div>
                @endif
            </div>
        </div>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-xl shadow-md p-6">
                <h2 class="text-lg md:text-xl font-semibold text-gray-900 mb-4 flex items-center">
                    <svg class="w-5 h-5 md:w-6 md:h-6 mr-2 md:mr-3 text-orange-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="space-y-3">
                        @foreach($excludedAssets as $index => $asset)
                            <div class="flex items-center space-x-3 p-3 bg-orange-50 rounded-lg border border-orange-200">
                                ...
                                <p class="text-sm text-gray-700 flex-1">{{ $asset }}</p>
                            </div>
                        @endforeach
                    </div>
                @else
                    <div class="bg-orange-50 rounded-lg p-4 border border-orange-200">
                        <div class="flex items-center space-x-3">
                            ...
                            <p class="text-sm text-gray-500 italic">No assets were excluded from this maintenance</p>
                        </div>
                    </div>
                @endif
            </div>
            ...
            <div class="bg-white rounded-xl shadow-md p-6">
                <h2 class="text-lg md:text-xl font-semibold text-gray-900 mb-4 flex items-center">
                    <svg class="w-5 h-5 md:w-6 md:h-6 mr-2 md:mr-3 text-indigo-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="space-y-4">
                    <div class="flex items-center space-x-3 p-3 bg-indigo-50 rounded-lg border border-indigo-200">
                        ...
                        <div>
                            ...
                            <p class="text-sm font-semibold text-gray-900">{{ \Carbon\Carbon::parse($maintenance->created_at)->format('M d, Y g:i A') }}</p>
                        </div>
                    </div>
                    <div class="flex items-center space-x-3 p-3 bg-indigo-50 rounded-lg border border-indigo-200">
                        <div class="bg-indigo-200 p-2 rounded-lg">
                        ...
                        <div>
                            ...
                            <p class="text-sm font-semibold text-gray-900">{{ \Carbon\Carbon::parse($maintenance->updated_at)->format('M d, Y g:i A') }}</p>
                        </div>
                    </div>
                </div>
            </div>
