		...
		<div class="bg-red-800 rounded-xl shadow-lg p-4 md:p-6 text-white">
			<div class="flex items-center justify-between">
				<div class="flex items-center">
					...
					<div>
						<h1 class="text-2xl md:text-3xl font-bold text-white mb-1 md:mb-2">Add Laboratory</h1>
						<p class="text-red-100 text-sm md:text-lg">Create a new laboratory for logging</p>
					</div>
				</div>
	...
	<div class="bg-white rounded-xl shadow-lg p-4 md:p-6">
		<form action="{{ route('laboratories.store') }}" method="POST" class="space-y-6">
				...
				<div class="bg-gray-50 border border-gray-200 rounded-lg p-4 md:p-5">
					<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
						<div>
							...
							<input type="text" name="number" value="{{ old('number') }}" required class="w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500 px-4 py-2" placeholder="e.g., 401">
						...
						<div>
							...
							<input type="text" name="name" value="{{ old('name') }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500 px-4 py-2" placeholder="e.g., Laboratory 401">
				...
				<div class="bg-gray-50 border border-gray-200 rounded-lg p-4 md:p-5">
					<div class="grid grid-cols-1 md:grid-cols-3 gap-4">
						<div>
							...
							<input type="text" name="building" value="{{ old('building') }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500 px-4 py-2" placeholder="e.g., Main Building">
							@error('building')<p class="text-sm text-red-600 mt-1">{{ $message }}</p>@enderror
						</div>
						<div>
							<label class="block text-sm font-medium text-gray-700 mb-1">Floor</label>
							<input type="text" name="floor" value="{{ old('floor') }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500 px-4 py-2" placeholder="e.g., 4">
							@error('floor')<p class="text-sm text-red-600 mt-1">{{ $message }}</p>@enderror
						</div>
						<div>
							<label class="block text-sm font-medium text-gray-700 mb-1">Room Number</label>
							<input type="text" name="room_number" value="{{ old('room_number') }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500 px-4 py-2" placeholder="e.g., 401">
			...
			<div class="flex justify-end space-x-2 pt-2">
				...
				<button type="submit" class="px-4 py-2 bg-red-800 text-white rounded-md hover:bg-red-700">Save Laboratory</button>
			</div>
		</form>
	</div>
</div>
