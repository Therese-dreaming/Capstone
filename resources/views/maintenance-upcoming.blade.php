...
<div class="flex-1 p-4 md:p-8 bg-gray-50">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-3 sm:space-y-0">
                <div class="flex items-center">
                    ...
                    <div class="min-w-0 flex-1">
                        <h1 class="text-xl sm:text-2xl md:text-3xl font-bold text-white mb-1 md:mb-2 truncate">Upcoming Maintenance</h1>
                        <p class="text-red-100 text-xs sm:text-sm md:text-lg">View and manage scheduled maintenance tasks</p>
                    </div>
                </div>
                <a href="{{ route('maintenance.schedule') }}" 
                   class="inline-flex items-center justify-center px-3 sm:px-4 py-2 bg-white/20 text-white font-medium rounded-lg hover:bg-white/30 focus:outline-none focus:ring-2 focus:ring-white/50 focus:ring-offset-2 focus:ring-offset-red-800 transition-colors duration-200 text-sm sm:text-base flex-shrink-0">
                    <svg class="w-4 h-4 sm:w-5 sm:h-5 mr-1 sm:mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
                    </svg>
                    <span class="hidden sm:inline">Schedule New Maintenance</span>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 md:gap-8">
        ...
        <div class="bg-white rounded-xl shadow-md overflow-hidden">
            <div class="flex justify-between items-center p-3 sm:p-4 md:p-6 bg-gray-50 cursor-pointer hover:bg-gray-100 transition-colors duration-200" onclick="toggleLab('location-{{ $locationId }}')">
                <h3 class="text-base sm:text-lg md:text-xl font-semibold text-gray-800 truncate pr-2">{{ $locationName }}</h3>
                <div class="flex items-center space-x-2 flex-shrink-0">
                    <span class="px-2 sm:px-3 py-1 bg-red-100 text-red-700 text-xs font-medium rounded-full whitespace-nowrap">
                        {{ $locationMaintenance->count() }} task{{ $locationMaintenance->count() !== 1 ? 's' : '' }}
                    </span>
                    <svg class="w-4 h-4 sm:w-5 sm:h-5 transform transition-transform duration-200 text-gray-500" id="chevron-{{ $locationId }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                    </svg>
                </div>
            </div>

            <div id="location-{{ $locationId }}" class="hidden p-3 sm:p-4 md:p-6 border-t border-gray-100">
                @php
                ...
                <div class="space-y-3 sm:space-y-4 md:space-y-6">
                    ...
                    <div class="border border-gray-200 rounded-lg overflow-hidden">
                        <div class="bg-gray-50 px-2 py-2 sm:px-3 sm:py-2 md:px-4 md:py-3 border-b border-gray-200 cursor-pointer hover:bg-gray-100 transition-colors duration-200" onclick="toggleDate('date-{{ str_replace('-', '', $date) }}-{{ $locationId }}')">
                            <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center">
                                <div class="mb-2 sm:mb-0">
                                    ...
                                    <p class="text-xs sm:text-sm text-gray-500">
                                        {{ $maintenanceItems->count() }} task{{ $maintenanceItems->count() !== 1 ? 's' : '' }} scheduled
                                    </p>
                                </div>
                                <div class="flex items-center justify-between sm:justify-end sm:space-x-2">
                                    <div class="flex flex-wrap gap-1 sm:gap-2">
                                        ...
                                        @if($userIsAssignedToAnyTask)
                                            <button onclick="completeAllTasks('{{ $locationId}}', '{{ $date }}'); event.stopPropagation();" 
                                                class="text-xs sm:text-sm px-2 py-1 sm:px-3 sm:py-1.5 bg-green-600 text-white rounded-lg hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 transition-colors duration-200 flex items-center shadow-sm">
                                                <svg class="w-3 h-3 sm:w-4 sm:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                                                </svg>
                                                <span class="ml-1 hidden sm:inline">All</span>
                                            </button>
                                        @else
                                            <button disabled 
                                                class="text-xs sm:text-sm px-2 py-1 sm:px-3 sm:py-1.5 bg-gray-400 text-white rounded-lg cursor-not-allowed flex items-center shadow-sm">
                                                <svg class="w-3 h-3 sm:w-4 sm:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                                                </svg>
                                                <span class="ml-1 hidden sm:inline">All</span>
                                        ...
                                        @if(auth()->user()->group_id == 1)
                                            <button onclick="cancelAllTasks('{{ $locationId}}', '{{ $date }}'); event.stopPropagation();" 
                                                class="text-xs sm:text-sm px-2 py-1 sm:px-3 sm:py-1.5 bg-red-600 text-white rounded-lg hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition-colors duration-200 flex items-center shadow-sm">
                                                <svg class="w-3 h-3 sm:w-4 sm:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    <svg class="w-4 h-4 sm:w-5 sm:h-5 transform transition-transform duration-200 ml-2 text-gray-500 flex-shrink-0" id="chevron-date-{{ str_replace('-', '', $date) }}-{{ $locationId}}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                                    </svg>
                                </div>
                            </div>
                        ...
                        <div id="date-{{ str_replace('-', '', $date) }}-{{ $locationId}}" class="hidden">
                            ...
                            <div class="divide-y divide-gray-100">
                                @php
                                $allExcludedAssets = $maintenanceItems->pluck('excludedAssetModels')->flatten()->unique('id');
                                @endphp
                                @if($allExcludedAssets->isNotEmpty())
                                    <div class="p-2 sm:p-3 md:p-4 bg-gray-50 border-l-4 border-blue-400">
                                        <p class="text-xs sm:text-sm text-gray-700 font-medium mb-2 flex items-center">
                                            <svg class="w-3 h-3 sm:w-4 sm:h-4 mr-2 text-blue-500 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                                            </svg>
                                            Excluded Assets for this Schedule:
                                        </p>
                                        <ul class="list-disc list-inside text-xs sm:text-sm text-gray-600 ml-3 sm:ml-4 space-y-1">
                                            @foreach($allExcludedAssets as $asset)
                                                <li class="break-words"><strong>{{ $asset->serial_number }}</strong> - {{ $asset->name }}</li>
                                            @endforeach
                                        </ul>
                                    </div>
                                @endif
                                ...
                                <div class="p-2 sm:p-3 md:p-4 hover:bg-gray-50 transition-colors duration-200 border-b border-gray-100 last:border-b-0">
                                    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-start space-y-3 sm:space-y-0">
                                        <div class="flex-1 min-w-0">
                                            <h4 class="font-medium text-gray-800 text-sm sm:text-base break-words">{{ $maintenance->maintenance_task }}</h4>
                                            <div class="flex items-center mt-2 space-x-2">
                                                <div class="bg-blue-50 p-1 rounded-full flex-shrink-0">
                                            ...
                                            <div class="flex space-x-1 sm:space-x-2">
                                                @if(auth()->user()->id == $maintenance->technician_id)
                                                    <button onclick="markAsComplete('{{ $maintenance->id }}', '{{ $locationId}}')" 
                                                        class="text-xs sm:text-sm px-2 py-1 sm:px-3 sm:py-1 bg-green-600 text-white rounded-lg hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 transition-colors duration-200 shadow-sm">
                                                        <svg class="w-3 h-3 sm:w-4 sm:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                                                        </svg>
                                                    </button>
                                                @else
                                                    <button disabled 
                                                        class="text-xs sm:text-sm px-2 py-1 sm:px-3 sm:py-1 bg-gray-400 text-white rounded-lg cursor-not-allowed shadow-sm">
                                                        <svg class="w-3 h-3 sm:w-4 sm:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                                                        </svg>
                                                    </button>
                                                ...
                                                @if(auth()->user()->group_id == 1)
                                                    <button onclick="cancelMaintenance('{{ $maintenance->id }}')" 
                                                        class="text-xs sm:text-sm px-2 py-1 sm:px-3 sm:py-1 bg-red-600 text-white rounded-lg hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition-colors duration-200 shadow-sm">
                                                        <svg class="w-3 h-3 sm:w-4 sm:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                                                        </svg>
                                                    </button>
    ...
    <div class="items-center px-4 py-3">
        <form id="cancelForm" method="POST">
            @csrf
            @method('PATCH')
            <div id="cancellationModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden overflow-y-auto h-full w-full z-50 p-4">
                <div class="relative top-10 sm:top-20 mx-auto p-4 sm:p-5 border w-full max-w-sm sm:max-w-md shadow-lg rounded-xl bg-white">
                    <div class="mt-2 sm:mt-3 text-center">
                        ...
                        <h3 class="text-base sm:text-lg leading-6 font-medium text-gray-900">Cancel Maintenance</h3>
                        <div class="mt-2 px-2 sm:px-7 py-3">
                            <p class="text-xs sm:text-sm text-gray-500 mb-4">Are you sure you want to cancel this maintenance task?</p>
                            
                            <!-- Notes Field -->
                            <div class="text-left">
                                <label class="block text-xs sm:text-sm font-medium text-gray-700 mb-2">Notes (optional)</label>
                                <textarea name="notes" rows="3" placeholder="Add any notes regarding cancellation (e.g., reason for cancellation)" class="w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500 px-3 py-2 text-xs sm:text-sm"></textarea>
                            </div>
                        </div>
                        <div class="flex flex-col sm:flex-row justify-center space-y-2 sm:space-y-0 sm:space-x-3 mt-4 sm:mt-6">
                            <button type="submit" 
                                    class="w-full sm:w-auto px-4 py-2 bg-red-600 text-white text-sm sm:text-base font-medium rounded-lg shadow-sm hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition-colors duration-200">
                                Confirm
                            </button>
                            <button type="button" onclick="hideModal('cancellationModal')" 
                                    class="w-full sm:w-auto px-4 py-2 bg-gray-100 text-gray-700 text-sm sm:text-base font-medium rounded-lg shadow-sm hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 transition-colors duration-200">
                                Cancel
                            </button>
                        </div>
                    </div>
