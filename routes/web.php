...
Route::get('/asset-list', function () {
	if (auth()->check()) {
		if (auth()->user()->group_id === 4) {
			return redirect()->route('custodian.assets.index');
		} else {
...
Route::middleware(['auth'])->group(function () {
	...
	Route::get('/notifications/go/{id}', [NotificationController::class, 'redirect'])->name('notifications.redirect');
	...
	Route::middleware([\App\Http\Middleware\CheckRole::class.':1'])->group(function () {
		...
		Route::prefix('reports')->group(function () {
			...
			Route::match(['get', 'post'], '/category/export-pdf', [ReportController::class, 'categoryReportExportPDF'])->name('reports.category.exportPDF');
			...
			Route::match(['get', 'post'], '/location/export-pdf', [ReportController::class, 'locationReportExportPDF'])->name('reports.location.exportPDF');
			...
			Route::match(['get', 'post'], '/procurement-history/export-pdf', [ReportController::class, 'procurementHistoryExportPDF'])->name('reports.procurement-history.export-pdf');
			...
			Route::match(['get', 'post'], '/disposal-history/export-pdf', [ReportController::class, 'disposalHistoryExportPDF'])->name('reports.disposal-history.export-pdf');
			...
			Route::match(['get', 'post'], '/vendor-analysis/export-pdf', [ReportController::class, 'vendorAnalysisExportPDF'])->name('reports.vendor-analysis.exportPDF');
			...
			Route::match(['get', 'post'], '/lab-usage/export', [ReportController::class, 'exportLabUsageToPdf'])->name('reports.lab-usage.export');
		...
		Route::resource('laboratories', \App\Http\Controllers\LaboratoryController::class)->except(['show']);
	});
	...
	Route::middleware([\App\Http\Middleware\CheckRole::class.':1,2'])->group(function () {
		...
		Route::get('/employee-performance/{user}/metrics', [ReportController::class, 'employeeMetrics'])->name('employee-performance.metrics');
		...
		Route::patch('/maintenance/{id}/cancel', [MaintenanceController::class, 'cancel'])->name('maintenance.cancel');
	...
	// Custodian routes (group_id = 4) - Asset management only
	Route::middleware([\App\Http\Middleware\CheckRole::class.':4'])->group(function () {
		// Asset management routes for Custodians
		Route::get('/custodian/assets', [AssetController::class, 'index'])->name('custodian.assets.index');
		Route::get('/custodian/assets/create', [AssetController::class, 'create'])->name('custodian.assets.create');
		Route::post('/custodian/assets', [AssetController::class, 'store'])->name('custodian.assets.store');
		Route::get('/custodian/assets/{asset}', [AssetController::class, 'show'])->name('custodian.assets.show');
		Route::get('/custodian/assets/{asset}/edit', [AssetController::class, 'edit'])->name('custodian.assets.edit');
